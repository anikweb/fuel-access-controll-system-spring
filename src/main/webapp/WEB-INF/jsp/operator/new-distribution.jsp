...
<my:panelLayout title="নতুন বিতরণ | FACS">
    ...
    <jsp:body>
        <section class="w-full max-w-3xl mx-auto flex flex-col gap-6">
            ...
            <header>
                <h1 class="text-[28px] sm:text-[32px] font-bold text-brand tracking-tight leading-snug">নতুন বিতরণ</h1>
                <p class="mt-2 text-sm text-gray-500">লাইসেন্স প্লেটের ছবি আপলোড করুন। সিস্টেম যানবাহনের তথ্য যাচাই করবে।</p>
            </header>

            <c:if test="${not empty error}">
                <div class="rounded-lg border border-red-200 bg-red-50 text-brand-red px-4 py-3 text-sm font-medium">
                    <c:out value="${error}"/>
                </div>
            </c:if>

            <form action="<c:url value='/operator/transactions/verify'/>" method="post" enctype="multipart/form-data"
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <label for="photo"
                        ...
                        <div class="flex flex-col items-center gap-3" data-upload-placeholder>
                            <span class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-brand/10 text-brand [&>svg]:w-7 [&>svg]:h-7">
                                <my:icon name="camera"/>
                            </span>
                            <p class="text-base font-semibold text-gray-700" data-upload-label>ছবি আপলোড করতে ক্লিক করুন বা টেনে আনুন</p>
                        ...
                        <span class="hidden absolute inset-0 items-center justify-center bg-black/45 text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition"
